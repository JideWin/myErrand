...
const JobDetailsScreen = ({ navigation, route }) => {
  ...
  const handleCompleteJob = () => {
    // --- THIS IS THE FIX ---
    // We MUST pass 'taskerId' so the Payment Screen knows who to pay!
    if (!job.assignedTaskerId) {
      Alert.alert("Error", "No Tasker assigned to this job.");
      return;
    }
    ...
    navigation.navigate("Payment", {
      ...
      taskerId: job.assignedTaskerId, // <--- CRITICAL: Sending the ID here
    });
  };
